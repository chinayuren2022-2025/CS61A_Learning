...
def is_always_roll(strategy, goal=GOAL):
    ...
    set_roll = strategy(player_score, opponent_score)
    while player_score < goal and opponent_score < goal:
        while player_score < goal and opponent_score < goal:
            if strategy(player_score, opponent_score) != set_roll:
                is_it = False
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    """
    # BEGIN PROBLEM 9
    num_rolls, best_score, best_num = 1, 1, 1
    test_dice = make_averaged(roll_dice, times_called)
    while num_rolls <= 10:
        score = test_dice(num_rolls, dice)
        if score > best_score:
            best_num = num_rolls
            best_score = score
